...
Gemini APIモードでは IMG-R-05 / IMG-R-09 / IMG-W-01 に対して、画像が取得可能な場合はテキストPartと画像inlineDataを同じ `generateContent` リクエストに含める。応答JSONは `altAssessment` と `suggestedAlt` を含め、使用履歴へ画像評価情報を残す。画像取得に失敗した場合は `image_unresolved` として記録し、テキストのみ評価へフォールバックする。
