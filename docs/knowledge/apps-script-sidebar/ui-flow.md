...
「検証」タブでは、現在の出力HTMLを取り込み、`sample-expected-auto` または `sample-expected-reviewed` と正規化比較する。差分がある場合は、標準レビュー手順に沿って iframe title、caption、リンク文言、img alt、SKIP-04削除を確認する。

## 画像/alt確認

画像候補カードにはプレビュー、src、現在のalt、alt妥当性、判定理由、推奨altを表示する。相対画像はベースURL入力で解決する。
