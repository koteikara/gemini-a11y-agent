...
- APIキー未設定時にAPI設定タブと候補カードで案内が表示される。
- APIキー保存後もキー文字列が再表示されない。
- APIキー削除後、API候補生成ボタンが無効になる。
- LINK-R-02候補でAPI候補生成を行い、JSON検証後に入力欄へ反映される。
- ユーザーが「HTMLへ適用」を押すまで出力HTMLは確定変更されない。
