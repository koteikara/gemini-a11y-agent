...
- a11y hybrid detect/fix ルール: [`docs/a11y-hybrid-detect-fix-rules.md`](a11y-hybrid-detect-fix-rules.md)
  - report-only 検出ロジックの確認コマンド: `python tools/check_hybrid_rules.py` と `python tools/check_hybrid_rule_detection.py`
  - `tests/test_hybrid_rules_report_only.py` は候補抽出、`auto_fix=False`、入力HTML非変更のみを確認します。自動修正はまだ有効化しておらず、HTML出力、fixture、gold、ai-v1.0 出力は変更しません。
